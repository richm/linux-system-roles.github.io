...
```
The ovirt logging_inputs are divided into 3 subtypes, `collectd`, `engine` and `vdsm`. They are separately configured to allow each subtype send its log messages to the different logging_outputs using logging_flows. The configuration files are generated by the ovirt task [Create oVirt input configuration files in /etc/rsyslog.](https://github.com/linux-system-roles/logging/blob/main/roles/rsyslog/tasks/inputs/ovirt/main.yml) using [ovirt_input_template.j2](https://github.com/linux-system-roles/logging/blob/main/roles/rsyslog/templates/ovirt_input_template.j2).
...
There are 2 test playbooks [tests_ovirt_elasticsearch.yml](https://github.com/linux-system-roles/logging/blob/main/tests/tests_ovirt_elasticsearch.yml) and [tests_ovirt_elasticsearch_params.yml](https://github.com/linux-system-roles/logging/blob/main/tests/tests_ovirt_elasticsearch_params.yml). The test playbooks configure a basic input and three ovirt inputs, a default files output and two elasticsearch outputs, and a flow from the ovirt inputs to the elasticsearch outputs and the basic input to the default files output.
